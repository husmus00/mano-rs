...
pub fn pass_one(storage: &mut Storage, messages: &mut Messages) {
            ...
            if line.contains(',') {
                if let Some(comma_location) = line.find(',') {
                    ...
                    if PSEUDO_INSTRUCTIONS.contains(&label.as_str()) {
                        ...
                        add_detailed_error(&storage.source_program, i as i32, "", "", messages);
                        return;
...
pub fn pass_two(machine: &mut Storage, messages: &mut Messages) {
